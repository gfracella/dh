...
public class RebuildModel {
	...
	public static List<String> GetDailyTables() throws Exception {
		...
		try {
			...
			ResultSet rset = stmt.executeQuery(
					"SELECT REPLACE(REPLACE(PROCEDURE_NAME, 'REBUILD_',''), '_BDT') TAB FROM USER_PROCEDURES WHERE OBJECT_NAME = 'REBUILD_BDT' AND PROCEDURE_NAME LIKE 'REBUILD_%' AND PROCEDURE_NAME <> 'REBUILD_ALL_BDT' ORDER BY SUBPROGRAM_ID");
			while (rset.next()) {
	...
	public static String CreaRebuildProc() throws Exception {
		...
		sql += String.format(
				"------------------------------------------------------------------------------------------------------------\n");
		sql += String.format(
				"--- FUNZIONE get_Default_rebuild_days(123): calcola il numero di giorni di default se non specificato ----\n");
		sql += String.format(
				"------------------------------------------------------------------------------------------------------------\n");
		sql += String.format("\n");
		sql += String.format("FUNCTION get_Default_rebuild_days (nrofdays IN INT) RETURN INT\n");
		...
		sql += String.format(
				"    --   WHERE owner = 'S2A'  and object_name = 'REBUILD_BDT' AND object_name <> 'REBUILD_ALL_BDT' and procedure_name like 'REBUILD*BDT')\n");
		sql += String.format("    --  LOOP\n");
		sql += String.format(
				"    --     v_sql := 'BEGIN ' || bRow.owner || '.' || bRow.object_name || '.' || bRow.procedure_name || '(' || nrofdays || '); END;';\n");
		...
		for (int i = 0; i < aTabs.size(); i++) {
			...
			sql += String.format(
					"    b_log_Id varchar2(32) := ' [' || to_char(current_timestamp, 'HH24') || to_char(current_timestamp, 'MM') || to_char(current_timestamp, 'FF') || '] '; \n\n");
			...
			if (!tabObj.getName().equals("SD51T")) {
				...
				sql += String.format(
						"  Calcola la data relativa all'ultima importazione FULL. Viene presa come data di ultima importazione FULL quella relativa\n");
				sql += String.format(
						"  alla data MASSIMA del mese precedente rispetto alla data passata in input. Se non esiste una importazione FULL per il mese\n");
				sql += String.format(
						"  precedente viene presa come data di ultima importazione FULL quella relativa alla data MINIMA del mese delle data passata in input.\n");
				sql += String.format(
						"***************************************************************************************************************************************/\n");

				sql += String.format(
						"FUNCTION CALCULATE_LAST_FULL_IMPORT_DATE(anAbiIn VARCHAR2, aDatain DATE) RETURN DATE IS\n");
				...
				sql += String.format(
						"    --Se aDataIn e' un fine mese prendo come data di ultima importazione FULL la data massima del mese di aDatain\n");

				sql += String.format(
						"    SELECT MAX(%s) INTO bLastFull FROM %s WHERE %s  BETWEEN TRUNC(aDatain, 'FMMM') AND LAST_DAY(aDatain) and %s=anAbiIn;\n",
						tabObj.getDTVField(), tabObj.getName(), tabObj.getDTVField(), tabObj.getAbiField());

				sql += String.format("  ELSE\n");
				sql += String.format(
						"    --Se aDataIn NON un fine mese prendo come ultima importazione FULL la data massima del mese precente a quello passato come aDatain\n");

				sql += String.format(
						"    SELECT MAX(%s) INTO bLastFull FROM %s WHERE %s  BETWEEN TRUNC(ADD_MONTHS(aDatain, -1), 'FMMM') AND LAST_DAY(ADD_MONTHS(aDatain, -1)) and %s=anAbiIn;\n",
						tabObj.getDTVField(), tabObj.getName(), tabObj.getDTVField(), tabObj.getAbiField());

				sql += String.format("  END IF;\n");
				sql += String.format(
						"-- Se non esiste l'ultimo FULL del mese precente prendo come ultimmo FULL la data minima del mese corrente\n");
				sql += String.format("IF bLastFull IS NULL THEN\n");

				sql += String.format(
						"    SELECT MIN(%s) INTO bLastFull FROM %s WHERE %s  BETWEEN TRUNC(aDatain, 'FMMM') AND LAST_DAY(aDatain) and %s=anAbiIn;\n",
						tabObj.getDTVField(), tabObj.getName(), tabObj.getDTVField(), tabObj.getAbiField());

				sql += String.format("END IF;\n");
				sql += String.format(
						"dbms_output.put_line('REBUILD_%s_BDT.CALCULATE_LAST_FULL_IMPORT_DATE. ABI: ' || anAbiIn ||  ' aDataIn:' || aDatain || '. bLastFull is ' || bLastFull);\n",
						tabObj.getName());
			...
			sql += String.format(
					"  Calcola il numero di giorni di cui fare la rebuild: ultimo giorno della _BDT o ultimo giorno della tabella o il default \n");
			sql += String.format(
					"***************************************************************************************************************************************/\n");
			...
			sql += String.format("        END IF;\n");
			sql += String.format(
					"        SELECT ABS(NVL(TRUNC(b_current_date-1) - TRUNC(b_startDate) , 0)) into b_days from dual; -- rebuild fino al giorno precedente\n");
			...
			sql += String.format(
					"b_current_date:=current_date + interval '5' hour;  -- Aggiunto 5 ore alla current_date (per fare in modo che le rebuild cominciate prima di mezzanotte elaborino il giorno successivo;\n");
			if (tabObj.getName().equals("SD49T"))
				sql += String.format(
						"b_current_date:=b_current_date-1;  -- Questa tabella viene spedita con i dati relativi a T-2\n");
			...
			sql += String.format(
					"    dbms_output.put_line('Rebuilding  %s_BDT. Numero giorni da ricostruire:' || b_nrOfDays );\n",
					tabObj.getName());

			sql += String.format(
					"    S2A.LOG_MANAGER.LOG_INFO('REBUILD_%s' || '-' || b_abi, b_log_id || 'Rebuilding  %s_BDT. Numero giorni da ricostruire:' || b_nrOfDays );\n",
					tabObj.getName(), tabObj.getName());

			sql += String.format("\n");
			sql += String.format(
					"-- A partire da b_nrOfDays giorni prima di oggi calcolo il record da inserire in %s_BDT\n",
					tabObj.getName());
			sql += String.format("    FOR idx IN REVERSE 1..b_nrOfDays LOOP\n");
			sql += String.format(
					"        b_ela_date := trunc(b_current_date) - idx; -- rebuild fino al giorno precedente\n");
			...
			sql += String.format("        DELETE FROM %s_bdt WHERE %s = b_abi AND cal_date = b_ela_date ; \n",
					tabObj.getName(), tabObj.getAbiField());
			sql += String.format(
					"        dbms_output.put_line('Rebuilding %s_BDT. Cancellazione dei record con data  ' || b_ela_date || ' e ABI:' || b_abi || '. Nr. record cancellati=' || SQL%%rowcount);\n",
					tabObj.getName());
			sql += String.format(
			...
			if (!tabObj.getName().equals("SD51T")) {
				sql += String.format("\n        -- Calcolo ultimo full \n");
				sql += String.format(
						"        b_last_full_import_date:=CALCULATE_LAST_FULL_IMPORT_DATE(b_abi, b_ela_date);\n");
			}
			sql += String.format("        INSERT INTO %s_bdt (\n", tabObj.getName());
			sql += String.format("            cal_date,\n");
			for (int j = 0; j < tabObj.getKeys().size() - 1; j++) {
				String key = tabObj.getKeys().get(j);
				if (!key.equals(tabObj.getDTVField()))
					sql += String.format("            %s,\n", key);
				// sql+=String.format(" s50abi,\n");
				// sql+=String.format(" s50ser,\n");
				// sql+=String.format(" s50rap,\n");
				// sql+=String.format(" s50dtv\n");
			}
			sql += String.format("            %s )\n", tabObj.getDTVField());
			// sql+=String.format( " )\n");
			...
			if (tabObj.getName().equals("SD51T"))
				...
				sql += String.format(
						"            WHERE %s = b_abi AND %s >= b_last_full_import_date and %s <= b_ela_date\n",
			...
			for (int j = 0; j < tabObj.getKeys().size() - 1; j++) {
				...
				// if (j == 0)
				// sql+=String.format( " GROUP BY %s,\n", tabObj.getAbiField());
				// sql+=String.format(" GROUP BY s50abi,\n");
				// sql+=String.format(" s50ser,\n");
				// sql+=String.format(" s50rap\n");
				// else if (j == tabObj._keys.size() - 2)
				// sql+=String.format( " %s\n", key);
				// else
				// sql+=String.format( " %s,\n", key);
			}
			...
			sql += String.format("            );\n");
			sql += String.format(
					"    dbms_output.put_line('Rebuilding %s_BDT. Date:' || b_ela_date || ' ABI:' || b_abi || '. Nr. record inseriti=' || SQL%%rowcount);\n",
					tabObj.getName());

			sql += String.format(
					"    S2A.LOG_MANAGER.LOG_INFO('REBUILD_%s' || '-' || b_abi, b_log_id || 'Rebuilding %s_BDT. Date:' || b_ela_date || ' ABI:' || b_abi || '. Nr. record inseriti=' || SQL%%rowcount);\n",
					tabObj.getName(), tabObj.getName());
